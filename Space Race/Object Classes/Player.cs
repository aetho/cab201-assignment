	...
	public class Player {
		...
		public void Play(Die d1, Die d2) {
			...
			if (hasPower) {
				...
				// Make sure player does not over-shoot the finishing block
				if (position > Board.FINISH_SQUARE_NUMBER) position = Board.FINISH_SQUARE_NUMBER;
		...
		private bool ReachedFinalSquare() {
			return position == Board.FINISH_SQUARE_NUMBER;
